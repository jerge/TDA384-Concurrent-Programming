...
-export([request/2, request/3, start/3, stop/1,
  update/2]).
...
stop(Atom) -> Atom ! stop, catch unregister(Atom), ok.
  ...
  receive
    {request, From, Ref, Data} ->
      case catch F(State, Data) of
        {'EXIT', Reason} ->
          From ! {exit, Ref, Reason}, loop(State, F);
        {reply, R, NewState} ->
...
request(Pid, Data) -> request(Pid, Data, 3000).
